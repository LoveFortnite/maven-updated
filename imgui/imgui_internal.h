...
struct ImGuiWindowoptions
{
	...
	ImGuiWindowoptions() { Name = NULL; Id = 0; Pos = Size = ImVec2(0, 0); Collapsed = false; }
};

struct ImGuioptionsHandler
{
	const char* TypeName;   // Short description stored in .ini file. Disallowed characters: '[' ']'  
	ImGuiID     TypeHash;   // == ImHash(TypeName, 0, 0)
	void*       (*ReadOpenFn)(ImGuiContext* ctx, ImGuioptionsHandler* handler, const char* name);
	void(*ReadLineFn)(ImGuiContext* ctx, ImGuioptionsHandler* handler, void* entry, const char* line);
	void(*WriteAllFn)(ImGuiContext* ctx, ImGuioptionsHandler* handler, ImGuiTextBuffer* out_buf);
	void*       UserData;

	ImGuioptionsHandler() { memset(this, 0, sizeof(*this)); }
};
...
struct ImGuiContext
{
	...
	float                   DragCurrentValue;                   // Currently dragged value, always float, not rounded by end-user precision options
	...
	ImVec2                  OsImePosRequest, OsImePosSet;       // Cursor position request & last passed to the OS Input Method Editor
																...
																// options
	float                          optionsDirtyTimer;          // Save .ini options on disk when time reaches zero
	ImVector<ImGuiWindowoptions>  optionsWindows;             // .ini options for ImGuiWindow
	ImVector<ImGuioptionsHandler> optionsHandlers;            // List of .ini options handlers
...
enum ImGuiItemFlags_
{
	...
	ImGuiItemFlags_ButtonRepeat = 1 << 1,  // false    // Button() will return true multiple times based on io.KeyRepeatDelay and io.KeyRepeatRate options.
...
struct IMGUI_API ImGuiDrawContext
{
	...
	// We store the current options outside of the vectors to increase memory locality (reduce cache misses). The vectors are rarely modified. Also it allows us to not heap allocate for short-lived windows which are not using those options.
...
namespace ImGui
{
	...
	IMGUI_API void                  MarkInioptionsDirty();
	IMGUI_API ImGuioptionsHandler* FindoptionsHandler(const char* type_name);
	IMGUI_API ImGuiWindowoptions*  FindWindowoptions(ImGuiID id);
