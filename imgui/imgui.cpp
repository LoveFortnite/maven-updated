...
- Init: retrieve the ImGuiIO structure with ImGui::GetIO() and fill the fields marked 'options': at minimum you need to set io.DisplaySize
...
// TODO: Fill others options of the io structure later.
...
A: The easiest way is to create a dummy window. Call Begin() with NoTitleBar|NoResize|NoMove|NoScrollbar|NoSavedoptions|NoInputs flag,
...
this is also useful to set yourself in the context of another window (to get/set other options)
...
static ImGuiWindowoptions* AddWindowoptions(const char* name);

static void             LoadInioptionsFromDisk(const char* ini_filename);
static void             LoadInioptionsFromMemory(const char* buf);
static void             SaveInioptionsToDisk(const char* ini_filename);
static void             SaveInioptionsToMemory(ImVector<char>& out_buf);
static void             MarkInioptionsDirty(ImGuiWindow* window);
...
void ImGui::NewFrame()
{
	...
	if (g.MovingWindowMoveId && g.MovingWindowMoveId == g.ActiveId)
	{
		...
		if (g.IO.MouseDown[0])
		{
			...
			{
				MarkInioptionsDirty(actually_moving_window);
	...
	// Delay saving options so we don't spam disk too much
	if (g.optionsDirtyTimer > 0.0f)
	{
		g.optionsDirtyTimer -= g.IO.DeltaTime;
		/*if (g.optionsDirtyTimer <= 0.0f)
		SaveInioptionsToDisk(g.IO.IniFilename);*/
	}
...
static void* optionsHandlerWindow_ReadOpen(ImGuiContext*, ImGuioptionsHandler*, const char* name)
{
	ImGuiWindowoptions* options = ImGui::FindWindowoptions(ImHash(name, 0));
	if (!options)
		options = AddWindowoptions(name);
	return (void*)options;
}

static void optionsHandlerWindow_ReadLine(ImGuiContext*, ImGuioptionsHandler*, void* entry, const char* line)
{
	ImGuiWindowoptions* options = (ImGuiWindowoptions*)entry;
	float x, y;
	int i;
	if (sscanf(line, "Pos=%f,%f", &x, &y) == 2)         options->Pos = ImVec2(x, y);
	else if (sscanf(line, "Size=%f,%f", &x, &y) == 2)   options->Size = ImMax(ImVec2(x, y), GImGui->Style.WindowMinSize);
	else if (sscanf(line, "Collapsed=%d", &i) == 1)     options->Collapsed = (i != 0);
}

static void optionsHandlerWindow_WriteAll(ImGuiContext* imgui_ctx, ImGuioptionsHandler* handler, ImGuiTextBuffer* buf)
{
	...
	{
		ImGuiWindow* window = g.Windows[i];
		if (window->Flags & ImGuiWindowFlags_NoSavedoptions)
			continue;
		ImGuiWindowoptions* options = ImGui::FindWindowoptions(window->ID);
		if (!options)
			options = AddWindowoptions(window->Name);
		options->Pos = window->Pos;
		options->Size = window->SizeFull;
		options->Collapsed = window->Collapsed;
	}

	// Write a buffer
	// If a window wasn't opened in this session we preserve its options
	buf->reserve(buf->size() + g.optionsWindows.Size * 96); // ballpark reserve
	for (int i = 0; i != g.optionsWindows.Size; i++)
	{
		const ImGuiWindowoptions* options = &g.optionsWindows[i];
		if (options->Pos.x == FLT_MAX)
			continue;
		const char* name = options->Name;
		if (const char* p = strstr(name, "###"))  // Skip to the "###" marker if any. We don't skip past to match the behavior of GetID()
			name = p;
		buf->appendf("[%s][%s]\n", handler->TypeName, name);
		buf->appendf("Pos=%d,%d\n", (int)options->Pos.x, (int)options->Pos.y);
...
void ImGui::Initialize()
{
	...
	ImGuioptionsHandler ini_handler;
	ini_handler.TypeName = "Window";
	ini_handler.TypeHash = ImHash("Window", 0, 0);
	ini_handler.ReadOpenFn = optionsHandlerWindow_ReadOpen;
	ini_handler.ReadLineFn = optionsHandlerWindow_ReadLine;
	ini_handler.WriteAllFn = optionsHandlerWindow_WriteAll;
	g.optionsHandlers.push_front(ini_handler);

	// Load .ini file
	IM_ASSERT(g.optionsWindows.empty());
	LoadInioptionsFromDisk(g.IO.IniFilename);
	g.Initialized = true;
}
...
void ImGui::Shutdown()
{
	...
	g.MovingWindow = NULL;
	for (int i = 0; i < g.optionsWindows.Size; i++)
		IM_DELETE(g.optionsWindows[i].Name);
	...
	g.optionsWindows.clear();
	g.optionsHandlers.clear();
...
ImGuiWindowoptions* ImGui::FindWindowoptions(ImGuiID id)
{
	ImGuiContext& g = *GImGui;
	for (int i = 0; i != g.optionsWindows.Size; i++)
		if (g.optionsWindows[i].Id == id)
			return &g.optionsWindows[i];
	return NULL;
}
...
{
	ImGuiContext& g = *GImGui;
	g.optionsWindows.push_back(ImGuiWindowoptions());
	ImGuiWindowoptions* options = &g.optionsWindows.back();
	options->Name = ImStrdup(name);
	options->Id = ImHash(name, 0);
	return options;
}
...
static void LoadInioptionsFromDisk(const char* ini_filename)
{
	...
	if (!file_data)
		return;
	LoadInioptionsFromMemory(file_data);
	ImGui::MemFree(file_data);
}

ImGuioptionsHandler* ImGui::FindoptionsHandler(const char* type_name)
{
	ImGuiContext& g = *GImGui;
	const ImGuiID type_hash = ImHash(type_name, 0, 0);
	for (int handler_n = 0; handler_n < g.optionsHandlers.Size; handler_n++)
		if (g.optionsHandlers[handler_n].TypeHash == type_hash)
			return &g.optionsHandlers[handler_n];
	return NULL;
}

// Zero-tolerance, no error reporting, cheap .ini parsing
static void LoadInioptionsFromMemory(const char* buf_readonly)
{
	...
	ImGuioptionsHandler* entry_handler = NULL;
	...
	for (char* line = buf; line < buf_end; line = line_end + 1)
	{
		...
		if (line[0] == '[' && line_end > line&& line_end[-1] == ']')
		{
			...
			entry_handler = ImGui::FindoptionsHandler(type_start);
...
{
	ImGuiContext& g = *GImGui;
	g.optionsDirtyTimer = 0.0f;
	if (!ini_filename)
		return;

	ImVector<char> buf;
	SaveInioptionsToMemory(buf);
...
static void SaveInioptionsToMemory(ImVector<char>& out_buf)
{
	...
	for (int handler_n = 0; handler_n < g.optionsHandlers.Size; handler_n++)
	{
		ImGuioptionsHandler* handler = &g.optionsHandlers[handler_n];
...
void ImGui::MarkInioptionsDirty()
{
	ImGuiContext& g = *GImGui;
	if (g.optionsDirtyTimer <= 0.0f)
		g.optionsDirtyTimer = g.IO.IniSavingRate;
}
...
void ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, bool override_previous_tooltip)
{
	...
	ImGuiWindowFlags flags = ImGuiWindowFlags_Tooltip | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedoptions | ImGuiWindowFlags_AlwaysAutoResize;
...
bool ImGui::BeginPopup(const char* str_id, ImGuiWindowFlags flags)
{
	...
	return BeginPopupEx(g.CurrentWindow->GetID(str_id), flags | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoSavedoptions);
}
...
bool ImGui::BeginPopupModal(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	...
	bool is_open = Begin(name, p_open, flags | ImGuiWindowFlags_Popup | ImGuiWindowFlags_Modal | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoSavedoptions);
...
bool ImGui::BeginPopupContextItem(const char* str_id, int mouse_button)
{
	...
	if (IsMouseReleased(mouse_button) && IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup))
		OpenPopupEx(id);
	return BeginPopupEx(id, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoSavedoptions);
}
...
static bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	...
	ImGuiWindowFlags flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedoptions | ImGuiWindowFlags_ChildWindow;
...
static ImGuiWindow* CreateNewWindow(const char* name, ImVec2 size, ImGuiWindowFlags flags)
{
	...
	// User can disable loading and saving of options. Tooltip and child windows also don't store options.
	if (!(flags & ImGuiWindowFlags_NoSavedoptions))
	{
		// Retrieve options from .ini file
		// Use SetWindowPos() or SetNextWindowPos() with the appropriate condition flag to change the initial position of a window.
		window->Pos = window->PosFloat = ImVec2(60, 60);

		if (ImGuiWindowoptions* options = ImGui::FindWindowoptions(window->ID))
		{
			SetWindowConditionAllowFlags(window, ImGuiCond_FirstUseEver, false);
			window->PosFloat = options->Pos;
			window->Pos = ImFloor(window->PosFloat);
			window->Collapsed = options->Collapsed;
			if (ImLengthSqr(options->Size) > 0.00001f)
				size = options->Size;
		}
	}
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
	...
	if (hovered)
	{
		if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
		{
			...
			// Relies on repeat logic of IsMouseClicked() but we may as well do it ourselves if we end up exposing finer RepeatDelay/RepeatRate options.
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedoptions;
...
bool ImGui::BeginMainMenuBar()
{
	...
	if (!Begin("##MainMenuBar", NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedoptions | ImGuiWindowFlags_MenuBar)
		|| !BeginMenuBar())
	{
		End();
		PopStyleVar(2);
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
	...
	if (menu_is_open)
	{
		...
		ImGuiWindowFlags flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoSavedoptions | ((window->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu)) ? ImGuiWindowFlags_ChildMenu | ImGuiWindowFlags_ChildWindow : ImGuiWindowFlags_ChildMenu);
